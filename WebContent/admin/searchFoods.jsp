        ...
        <div class="col-lg-12">
            <div class="panel panel-default wk-panel wk-table-tools-panel">
                ...
                <div class="panel-body">
	                ...
	                <button id="uploadBtn" type="button" class="btn btn-default wk-tool-btn">上传照片</button>
                 </div>
            </div>
        </div>
    </div>
			...
			<div class="panel panel-default  wk-panel">
				<table class="table table-striped table-hover">
					...
					<tbody>
						...
						<tr>
							<td><input id="grid_${var.id}" name="radio"
								type="radio" aria-label="选择" /></td>
							<td>${var.name}</td>
							<td>${var.item.name}</td>
							<td>${var.item.address}</td>
						</tr>
						</c:forEach>
					</tbody>
	...
	<div class="modal fade" tabindex="-1" role="dialog"
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				...
				<form id="uploadFoodForm" method="POST" enctype="multipart/form-data">
					<div class="modal-body">
						<div class="form-inline">  
	                            <div class="form-group">
	                                <label  class="control-label wk-filed-label">文件路径: </label>
	                                 <div class="input-group">
	                                    <input required="required" name="file" type="file" class="form-control wk-normal-input" />
	                                </div>
	                            </div>
	                       </div>
					</div>
					<div class="modal-footer wk-modal-footer">
						<button id="uploadBtnOK" type="submit"
							class="btn btn-default wk-btn" data-dismiss="modal">确定</button>
						<button type="button" class="btn btn-default wk-tool-btn"
							data-dismiss="modal">取消</button>
					</div>
	...
	$("#deleteBtn").on("click", function() {
        ...
        if (id != "") {
            ...
            $("#deleteBtnOK").on("click", function() {
                location.href = "<%=basePath%>deleteFood/" + id + ".html";
            });
        }
    });
     ...
     $("#updateBtn").on("click", function() {
        ...
        if (id != "") {
			location.href = "<%=basePath%>updateFood/" + id + ".html";
        }
    });
     ...
     $("#uploadBtn").on("click", function() {
         ...
         if (id != "") {
        	 $("#uploadBtnOK").on("click", function() {
        		 $("#uploadFoodForm").attr("action", "<%=basePath%>/saveFoodFile/" + id + ".html")
        	 	 $("#uploadFoodForm").submit();
        	 })
         }
    });
